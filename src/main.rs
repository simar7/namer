...
fn main() {
    ...
    let right = [
        ...
        // Singh Gurtej is an inventor in the fields of thin film processes and materials, VLSI and semiconductor device fabrication. He currently holds the seventh most number of U.S. utility patents.
        "singh",
    ];

    let mut rng = thread_rng();
    println!("{}-{}", left.choose(&mut rng).unwrap(), right.choose(&mut rng).unwrap());
}
